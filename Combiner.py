...
class CombinerCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        # ------------------------------------------------------------------------------------------------------
        # a partir de aqui metodo de prueba para formato json


        # # ---
        # Los patters de extraer los strings
        # # ---
        # pattern_block = '"combine.*?\}'
        # pattern_files = '/{0,2}files\":\[*(.+) *\]'
        # pattern_file = '/{0,2}*\"(.+) \"*' 
        # pattern_output= '/{0,2}output":*\"(.+)\" *'
        ...
        #     # loop cogiendo los elementos a concatenar
        #     # ---
        #     for m in re.finditer(pattern_files, temp_block):
        #         # print('found:', m.start(), m.end())
        #         temp_list =sublime.Region(m.start(), m.end())
        #         temp_list = self.view.substr(temp_list)
        #         temp_list= m.group(1)
        #         print("lista:")
        #         print(temp_list)


        #         for m in re.finditer(pattern_file, temp_list):
        #             # print('found:', m.start(), m.end())
        #             temp_name =sublime.Region(m.start(), m.end())
        #             temp_name = self.view.substr(temp_name)
        #             temp_name= m.group(1)
